<?php
/**
 * CEX API Client Test Suite
 *
 * Tests for the CEX.io Spot Trading API PHP client
 *
 * Run tests with: ./vendor/bin/phpunit tests/CEXTest.php
 * Or with composer: composer test
 */
...
class CEXTest extends PHPUnit\Framework\TestCase {
	...
	/**
	 * Test deprecated hashrate method
	 */
	public function testHashrateDeprecated() {
		$result = $this->cex->hashrate();

		$this->assertArrayHasKey('error', $result);
		$this->assertStringContainsString('GHash.io', $result['error']);
		$this->assertStringContainsString('discontinued', $result['error']);
	}

	/**
	 * Test deprecated workers method
	 */
	public function testWorkersDeprecated() {
		$result = $this->cex->workers();
...
class CEXMockTest extends PHPUnit\Framework\TestCase {
	...
	/**
	 * Test convert method (deprecated but functional)
	 */
	public function testConvert() {
		...
		$result = $this->cex->convert('BTC/USD', 2.0);

		$this->assertEquals(100000.00, $result['amnt']);
		$this->assertEquals('50000.00', $result['rate']);
	}
	...
	/**
	 * Test price_stats method (deprecated, uses candles)
	 */
	public function testPriceStats() {
		$mockResponse = array(
			array(1700000000, '50000', '51000', '49000', '50500', '100')
		);
		$this->cex->setMockResponse('get_candles', $mockResponse);

		$result = $this->cex->price_stats('BTC/USD', 24, 10);

		// Should call get_candles internally
		$this->assertEquals('get_candles', $this->cex->getLastAction());
		$params = $this->cex->getLastParams();
		$this->assertEquals('BTC-USD', $params['pair']);
		$this->assertEquals('1h', $params['resolution']);
...
class CEXIntegrationTest extends PHPUnit\Framework\TestCase {
	...
	public function testLiveOrderBook() {
		$result = $this->cex->order_book('BTC-USD', 5);
